...
void HotkeyManagerWindowsPlugin::Register(
  ...
  UINT fs_modifiers = GetFsModifiersFromString(modifiers) | MOD_NOREPEAT;
  ...
  if (!registered) {
    ...
    result->Error(
        ...
        "Windows recusou o registro do atalho global (erro " +
            std::to_string(error) + ").");
    return;
  }
...
void HotkeyManagerWindowsPlugin::Unregister(
  ...
  std::string identifier =
      std::get<std::string>(args.at(flutter::EncodableValue("identifier")));
  const auto hotkey = hotkey_id_map_.find(identifier);
  if (hotkey == hotkey_id_map_.end()) {
    result->Success(flutter::EncodableValue(true));
    return;
  }
  int32_t hotkey_id = hotkey->second;
  ::UnregisterHotKey(
      ::GetAncestor(registrar_->GetView()->GetNativeWindow(), GA_ROOT),
      hotkey_id);
