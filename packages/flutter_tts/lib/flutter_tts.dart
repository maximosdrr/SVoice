...
class FlutterTts {
  ...
  /// Plays an existing audio file through the selected Windows render device.
  /// This is used by SVoice for locally generated XTTS audio.
  Future<dynamic> playAudioFile(String path) async =>
      await _channel.invokeMethod('playAudioFile', path);
  ...
  /// Returns the audio render devices available to MediaPlayer on Windows.
  /// Each item contains a stable `id` and a user-facing `name`.
  Future<dynamic> get getAudioDevices async {
    final devices = await _channel.invokeMethod('getAudioDevices');
    return devices;
  }

  /// Selects the Windows audio render device used for TTS playback.
  ...
  Future<SpeechRateValidRange> get getSpeechRateValidRange async {
    final validRange = await _channel.invokeMethod(
