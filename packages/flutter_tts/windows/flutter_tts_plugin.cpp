...
namespace {
	...
	private:
		...
		winrt::fire_and_forget playAudioFile(const std::string, FlutterResult);
		void pause();
		void continuePlay();
		void stop();
	...
	winrt::fire_and_forget FlutterTtsPlugin::playAudioFile(
		...
		try {
			...
			isSpeaking = false;
			result->Error("audio_file_error", to_string(error.message()));
		}
	}
	...
	winrt::fire_and_forget FlutterTtsPlugin::getAudioDevices(FlutterResult result) {
		try {
			...
			for (const auto& device : devices) {
				...
				value[flutter::EncodableValue("id")] = flutter::EncodableValue(to_string(device.Id()));
				value[flutter::EncodableValue("name")] = flutter::EncodableValue(to_string(device.Name()));
				values.push_back(flutter::EncodableValue(value));
			}
	...
	winrt::fire_and_forget FlutterTtsPlugin::setAudioDevice(const std::string deviceId, FlutterResult result) {
		try {
			if (deviceId.empty()) {
				selectedAudioDevice = nullptr;
				mPlayer.AudioDevice(nullptr);
				result->Success(1);
				co_return;
			}
			auto device = co_await winrt::Windows::Devices::Enumeration::DeviceInformation::CreateFromIdAsync(to_hstring(deviceId));
			selectedAudioDevice = device;
			mPlayer.AudioDevice(device);
			result->Success(1);
		}
		catch (const winrt::hresult_error& error) {
			result->Error("audio_device_error", to_string(error.message()));
...
namespace {
	...
	void FlutterTtsPlugin::HandleMethodCall(
		...
		else if (method_call.method_name().compare("playAudioFile") == 0) {
			const flutter::EncodableValue arg = method_call.arguments()[0];
			if (std::holds_alternative<std::string>(arg)) {
				if (speaking()) stop();
				playAudioFile(std::get<std::string>(arg), std::move(result));
			}
			else result->Success(0);
		}
		...
		else if (method_call.method_name().compare("getAudioDevices") == 0) {
			getAudioDevices(std::move(result));
		}
		else if (method_call.method_name().compare("setAudioDevice") == 0) {
			const flutter::EncodableValue arg = method_call.arguments()[0];
			if (std::holds_alternative<std::string>(arg)) {
