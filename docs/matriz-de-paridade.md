...
| 11 | Histórico de frases | F (6 itens, clique repete), W (1 item) | Flutter mantém 6; widget mostra só a última. | Widget: até 8 itens, clique repete; persistido em `LocalSettings`. | ✅ implementado — balões no chat (12 itens, clique repete, persistido); modo compacto esconde o histórico |
...
| 19 | Modo de processamento (auto/GPU/CPU) | F (configurações), S (`POST /config`) | `auto`, `gpu`(CUDA), `cpu`. | `auto`, `cuda`, `directml`, `cpu` (`rocm` reservado); tela de diagnóstico. | ✅ implementado — `POST /config` com backends; seção Diagnóstico em Ajustes |
...
| `gamebar/SVoice.GameBar/*` (widget, `XttsBridgeClient`) | Mesmo projeto | Ampliado com chat em balões, modo compacto, painéis de vozes e ajustes (com diagnóstico) e cancelamento. |
